...
def process_file():
    ...
    if request.method == 'POST':
        ...
        if not os.path.exists(abs_file_path):
            print("user directory image not found")
        ...
        with open(abs_file_path, 'r') as f:
            ...
            if not orig_file_name:
                print("metadata file path is invalid")
        ...
        if not os.path.exists(original_file_path):
            print("original file path doesn't exit")
        ...
        print("needs to be a post reqest")
...
if __name__ == '__main__':
    ...
    c.execute('SELECT username FROM users WHERE username = "flag"')
    if not c.fetchone():
